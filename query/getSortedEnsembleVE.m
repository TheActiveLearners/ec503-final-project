function [ trained_indicies ] = getSortedEnsembleVE( sel_idx )
...
% VOTING ENTROPY
C = 3;
class_counts = horzcat(label_abs_sums, abs(label_abs_sums-C));
ve = sum((class_counts/C).*log(class_counts/C),2);
[~, trained_indicies] = sort(ve, 'ascend');
end
